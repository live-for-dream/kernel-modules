...
static struct inode *
aufs_get_inode(struct super_block *sb, int mode, dev_t dev)
{
	printk(KERN_ALERT"%s : %s %i line", __func__, __FILE__, __LINE__);
...
aufs_mkdir(struct inode *dir, struct dentry *dentry, int mode) 
{
    ...
    res = aufs_mknod(dir, dentry, mode | S_IFDIR, mode);
    if (!res) {
        dir->i_nlink++;
    }
...
static int
aufs_create(struct inode *dir, struct dentry *dentry, int mode) 
{
	printk(KERN_ALERT"%s : %s %i line", __func__, __FILE__, __LINE__);
...
static int
aufs_fill_super(struct super_block *sb, void *data, int silent)
{
...
aufs_get_sb(struct file_system_type *fs_type, int flag, 
    const char *dev_name, void * data, struct vfsmount *mnt)
{
    return get_sb_single(fs_type, flag, data, aufs_fill_super, mnt);
}
...
static int 
aufs_create_by_name(const char  *name, mode_t mode, 
...
struct dentry *
aufs_create_file(const char *name, mode_t mode, struct dentry *parent, 
...
struct dentry *
aufs_create_dir(const char *name, struct dentry *parent)
{
...
aufs_init(void)
{
    ...
    if (!retval) {
		printk(KERN_EMERG"%s : in aufs_init %i line", __FILE__, __LINE__);
		printk(KERN_ALERT"%s : in aufs_init %i line", __FILE__, __LINE__);
	...
	printk(KERN_EMERG"%s : in aufs_init %i line", __FILE__, __LINE__);
	printk(KERN_ALERT"%s : in aufs_init %i line", __FILE__, __LINE__);
    pslot = aufs_create_dir("my_aufs_dir1", NULL);
	printk(KERN_ALERT"%s : in aufs_init %i line", __FILE__, __LINE__);
    /*aufs_create_file("file1", S_IFREG | S_IRUGO, pslot, NULL, NULL);
	printk(KERN_ALERT"%s : in aufs_init %i line", __FILE__, __LINE__);
    aufs_create_file("file2", S_IFREG | S_IRUGO, pslot, NULL, NULL);
    aufs_create_file("file3", S_IFREG | S_IRUGO, pslot, NULL, NULL);
    aufs_create_file("file4", S_IFREG | S_IRUGO, pslot, NULL, NULL);

    pslot = aufs_create_dir("my_aufs_dir2", NULL);
    aufs_create_file("file1", S_IFREG | S_IRUGO, pslot, NULL, NULL);
    aufs_create_file("file2", S_IFREG | S_IRUGO, pslot, NULL, NULL);
    aufs_create_file("file3", S_IFREG | S_IRUGO, pslot, NULL, NULL);
    aufs_create_file("file4", S_IFREG | S_IRUGO, pslot, NULL, NULL);
